<div>
    <x-slot name="header">
        <div class="flex justify-between sm:items-center sm:flex-row flex-col">
            ...
            <x-breadcumb>
                <li class="text-neutral-500 dark:text-neutral-400 ">Data Promo</li>
            </x-breadcumb>
        </div>
    </x-slot>
                        ...
                        <table class="min-w-full border text-left text-sm font-light">
                            ...
                            <tbody>
                                @forelse ($banners as $index => $banner)
                                    @livewire('promo.single', ['banner' => $banner, 'index' => $index + $banners->firstItem()], key($banner->id))
                                @empty
                                @endforelse
                            </tbody>
                        </table>
    ...
    @foreach ($banners as $banner)
        <x-modal name="confirm-banner-deletion-{{ $banner->id }}" focusable>
            <form wire:submit.prevent="delete({{ $banner->id }})">
                <div class="p-6">
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        {{ __('apakah anda yakin menghapus data promo ini?') }}
                    </p>
